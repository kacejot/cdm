...
class OrCondition : public Condition<typename std::remove_reference_t<T>::ItemType>
{
public:
...
template<typename T, typename U>
OrCondition<T, U>::OrCondition(T&& first, U&& second) :
...
template<typename T, typename U>
bool OrCondition<T, U>::match(const ItemType& item) const
{
